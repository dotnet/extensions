...
namespace System.Net.ServerSentEvents
{
    ...
    internal sealed class SseParser<T>
    {
        ...
        /// <summary>Initializes a new instance of the <see cref="SseParser{T}"/> class.</summary>
        ...
        /// <exception cref="InvalidOperationException">The parser has already been enumerated. May propagate out of a call to <see cref="IAsyncEnumerator{T}.MoveNextAsync"/>.</exception>
        /// <exception cref="OperationCanceledException">The enumeration was canceled. May propagate out of a call to <see cref="IAsyncEnumerator{T}.MoveNextAsync"/>.</exception>
        public async IAsyncEnumerable<SseItem<T>> EnumerateAsync([EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    // Read more data into the buffer.
                    _ = await FillLineBufferAsync(cancellationToken).ConfigureAwait(false);
                }
            }
            finally
            {
        ...
        private bool ProcessLine(out SseItem<T> sseItem, out int advance)
        {
            ...
            else if (fieldName.SequenceEqual("event"u8))
            {
                ...
                _eventType = Helpers.Utf8GetString(fieldValue);
            }
            else if (fieldName.SequenceEqual("id"u8))
            {
                ...
                if (fieldValue.IndexOf((byte)'\0') < 0)
                {
                    ...
                    LastEventId = _eventId = Helpers.Utf8GetString(fieldValue);
                }
            }
            else if (fieldName.SequenceEqual("retry"u8))
            {
                ...
                if (long.TryParse(
                    ...
                    fieldValue,
#else
                    Helpers.Utf8GetString(fieldValue),
#endif
                    NumberStyles.None, CultureInfo.InvariantCulture, out long milliseconds) &&
                    milliseconds >= 0 && milliseconds <= TimeSpan_MaxValueMilliseconds)
                {
        ...
        private void ThrowIfNotFirstEnumeration()
        {
            ...
            {
                ThrowHelper.ThrowInvalidOperationException_EnumerateOnlyOnce();
            }
        }
