...
{
    private readonly ConditionalWeakTable<AIFunctionDefinition, object> _functionSchemaCache = new();
    private readonly ConditionalWeakTable<ChatResponseFormatJson, object> _responseFormatCache = new();

    private readonly ConditionalWeakTable<AIFunctionDefinition, object>.CreateValueCallback _functionSchemaCreateValueCallback;
    ...
    [EditorBrowsable(EditorBrowsableState.Never)] // functionality for AIFunction is satisfied by AIFunctionDefinition overload
    public JsonElement GetOrCreateTransformedSchema(AIFunction function) =>
        GetOrCreateTransformedSchema((AIFunctionDefinition)function);

    /// <summary>
    /// Gets or creates a transformed JSON schema for the specified <see cref="AIFunctionDefinition"/> instance.
    /// </summary>
    /// <param name="function">The function whose JSON schema we want to transform.</param>
    /// <returns>The transformed JSON schema corresponding to <see cref="TransformOptions"/>.</returns>
    public JsonElement GetOrCreateTransformedSchema(AIFunctionDefinition function)
    {
