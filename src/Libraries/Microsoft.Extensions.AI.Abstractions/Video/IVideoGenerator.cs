...
{
    /// <summary>
    /// Submits a video generation request and returns a <see cref="VideoGenerationOperation"/> that can be used to
    /// monitor progress, wait for completion, and download the generated content.
    /// </summary>
    /// <param name="request">The video generation request containing the prompt and optional media inputs.</param>
    ...
    Task<VideoGenerationOperation> GenerateAsync(VideoGenerationRequest request, VideoGenerationOptions? options = null, CancellationToken cancellationToken = default);
