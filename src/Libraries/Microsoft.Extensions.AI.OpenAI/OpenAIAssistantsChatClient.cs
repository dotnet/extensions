        ...
        IEnumerable<ChatMessage> messages, ChatOptions? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var update in updates.ConfigureAwait(false))
        {
            switch (update)
            {
                ...
                case RunStepDetailsUpdate details when !string.IsNullOrEmpty(details.CodeInterpreterInput) || details.CodeInterpreterOutputs is { Count: > 0 }:
                    CodeExecutionResultContent codeInterpreterContent = new()
                    {
                        RawRepresentation = details
                    };

                    if (details.CodeInterpreterInput is string input && input.Length > 0)
                    {
                        (codeInterpreterContent.Inputs ??= []).Add(new TextContent(input));
                    }

                    if (details.CodeInterpreterOutputs is { Count: > 0 })
                    {
                        foreach (var codeInterpreterOutput in details.CodeInterpreterOutputs)
                        {
                            if (codeInterpreterOutput.ImageFileId is not null)
                            {
                                (codeInterpreterContent.Outputs ??= []).Add(new HostedFileContent(codeInterpreterOutput.ImageFileId));
                            }

                            if (codeInterpreterOutput.Logs is string logs)
                            {
                                (codeInterpreterContent.Outputs ??= []).Add(new TextContent(logs));
                            }
                        }
                    }

                    yield return new ChatResponseUpdate(ChatRole.Assistant, [codeInterpreterContent])
                    {
                        AuthorName = _assistantId,
                        ConversationId = threadId,
                        MessageId = responseId,
