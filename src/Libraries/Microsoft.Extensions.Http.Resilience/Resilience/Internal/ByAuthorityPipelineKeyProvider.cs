...
internal sealed class ByAuthorityPipelineKeyProvider : IPipelineKeyProvider
{
    private readonly Redactor _redactor;
    private readonly ConcurrentDictionary<(string scheme, string host, int port), string> _cache = new();

    public ByAuthorityPipelineKeyProvider(Redactor redactor, DataClassification _)
    {
        _redactor = redactor;
    }
    ...
    public string GetPipelineKey(HttpRequestMessage requestMessage)
    {
        ...
        if (string.IsNullOrEmpty(pipelineKey))
        {
            Throw.InvalidOperationException(
                $"The redacted pipeline is an empty string and cannot be used for pipeline selection. Is redaction correctly configured?");
        }
