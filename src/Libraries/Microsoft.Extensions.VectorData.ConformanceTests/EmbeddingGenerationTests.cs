...
public abstract class EmbeddingGenerationTests<TKey>(
    EmbeddingGenerationTests<TKey>.StringVectorFixture stringVectorFixture,
    EmbeddingGenerationTests<TKey>.RomOfFloatVectorFixture romOfFloatVectorFixture)
    where TKey : notnull
{
    #region Search
    ...
    public virtual async Task SearchAsync_with_property_generator()
    {
        ...
        var collection = GetCollection<Record>(storeGenerator: true, collectionGenerator: true, propertyGenerator: true);
    ...
    {
        // Property level: embedding generators are defined at all levels. The property generator should take precedence.
        var collection = GetDynamicCollection(storeGenerator: true, collectionGenerator: true, propertyGenerator: true);
    ...
    public virtual async Task SearchAsync_with_collection_generator()
    {
        ...
        var collection = GetCollection<Record>(storeGenerator: true, collectionGenerator: true, propertyGenerator: false);
    ...
    public virtual async Task SearchAsync_with_store_generator()
    {
        ...
        var collection = GetCollection<Record>(storeGenerator: true, collectionGenerator: false, propertyGenerator: false);
    ...
    public virtual async Task UpsertAsync_batch()
    {
        ...
        await stringVectorFixture.TestStore.WaitForDataAsync(collection, 2, filter: r => r.Counter == counter1 || r.Counter == counter2);
    ...
    private VectorStoreCollection<TKey, TRecord> GetCollection<TRecord>(
        ...
        where TRecord : class
    {
        var (vectorStore, recordDefinition) = GetStoreAndRecordDefinition(storeGenerator, collectionGenerator, propertyGenerator);
    ...
    private (VectorStore vectorStore, VectorStoreCollectionDefinition definition) GetStoreAndRecordDefinition(
        bool storeGenerator = false,
        bool collectionGenerator = false,
        bool propertyGenerator = false)
    {
