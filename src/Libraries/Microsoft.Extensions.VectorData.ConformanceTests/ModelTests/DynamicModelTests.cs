    ...
    where TKey : notnull
{
    ...
    public virtual async Task Update_single_record()
    {
        ...
        Assert.NotNull(await Collection.GetAsync((TKey)existingRecord[KeyPropertyName]!));
        await Collection.UpsertAsync(updated);

        var received = await Collection.GetAsync((TKey)existingRecord[KeyPropertyName]!, new() { IncludeVectors = true });
    ...
    public abstract class Fixture : DynamicVectorStoreCollectionFixture<TKey>
    {
        ...
        protected override VectorStoreCollection<object, Dictionary<string, object?>> GetCollection()
            => TestStore.CreateDynamicCollection(CollectionName, CreateRecordDefinition());
        ...
        [
            new()
            {
                [KeyPropertyName] = GenerateNextKey<TKey>(),
