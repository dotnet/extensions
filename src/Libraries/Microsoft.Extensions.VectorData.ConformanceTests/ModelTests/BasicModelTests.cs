    ...
    where TKey : notnull
{
    ...
    public virtual async Task GetAsync_multiple_records(bool includeVectors)
    {
        ...
        var received = await Collection.GetAsync(ids, new() { IncludeVectors = includeVectors }).ToArrayAsync();
    ...
    public virtual async Task GetAsync_throws_for_null_key()
    {
        ...
        ArgumentNullException ex = await Assert.ThrowsAsync<ArgumentNullException>(() => Collection.GetAsync((TKey)default!));
    ...
    {
        ArgumentNullException ex = await Assert.ThrowsAsync<ArgumentNullException>(() => Collection.GetAsync(keys: null!).ToArrayAsync().AsTask());
    ...
    {
        Assert.Empty(await Collection.GetAsync([]).ToArrayAsync());
    }
    ...
    public virtual async Task GetAsync_with_filter_by_true()
    {
        ...
        var count = await Collection.GetAsync(r => true, top: 100).CountAsync();
    ...
    public virtual async Task GetAsync_with_filter_and_OrderBy()
    {
        ...
        results = await Collection.GetAsync(
                r => r.Number > 1,
                top: 2,
    ...
    public virtual async Task Insert_single_record()
    {
        ...
        await fixture.TestStore.WaitForDataAsync(Collection, recordCount: fixture.TestData.Count + 1);
    }
    ...
    public virtual async Task Update_single_record()
    {
        ...
        var received = await Collection.GetAsync(existingRecord.Key, new() { IncludeVectors = true });
    ...
    public virtual async Task Insert_multiple_records()
    {
        ...
        await Collection.UpsertAsync(newRecords);
    ...
    public virtual async Task Update_multiple_records()
    {
        ...
        await Collection.UpsertAsync(existingRecords);
    ...
    {
        ArgumentNullException ex = await Assert.ThrowsAsync<ArgumentNullException>(() => Collection.UpsertAsync(records: null!));
    ...
    public virtual async Task UpsertAsync_does_nothing_for_empty_batch()
    {
        ...
        var beforeCount = await Collection.GetAsync(r => true, top: 100).CountAsync();
        await Collection.UpsertAsync([]);
        var afterCount = await Collection.GetAsync(r => true, top: 100).CountAsync();
    ...
    public virtual async Task Delete_multiple_records()
    {
        ...
        await Collection.DeleteAsync(keysToRemove);
        Assert.Empty(await Collection.GetAsync(keysToRemove).ToArrayAsync());
    }
    ...
    {
        var beforeCount = await Collection.GetAsync(r => true, top: 100).CountAsync();
        await Collection.DeleteAsync(fixture.GenerateNextKey<TKey>());
        var afterCount = await Collection.GetAsync(r => true, top: 100).CountAsync();
    ...
    {
        ArgumentNullException ex = await Assert.ThrowsAsync<ArgumentNullException>(() => Collection.DeleteAsync(keys: null!));
        Assert.Equal("keys", ex.ParamName);
    }
    ...
    public sealed class Record : TestRecord<TKey>
    {
        ...
        public void AssertEqual(Record? other, bool includeVectors, bool compareVectors)
        {
            ...
            if (includeVectors)
            {
                Assert.Equal(Vector.Span.Length, other.Vector.Span.Length);

                if (compareVectors)
                {
                    Assert.Equal(Vector.ToArray(), other.Vector.ToArray());
                }
            }
            else
            {
