    ...
    where TRecord : class
{
    ...
    public virtual void CanRegisterVectorStore(ServiceLifetime lifetime, object? serviceKey)
    {
        foreach (var registrationDelegate in StoreDelegates)
        {
            IServiceCollection services = CreateServices(serviceKey);
...
#pragma warning restore CA2000 // Dispose objects before losing scope
        ...
        PopulateConfiguration(configuration, serviceKey);
    ...
    {
        TService? serviceFromFirstScope;
        TService? serviceFromSecondScope;
        TService? secondServiceFromSecondScope;
    ...
    private static TService Resolve<TService>(IServiceProvider serviceProvider, object? serviceKey = null)
    ...
    private sealed class FakeVectorStore : VectorStore
    {
        ...
        public override Task EnsureCollectionDeletedAsync(string name, CancellationToken cancellationToken = default)
            => throw new NotSupportedException();
        public override VectorStoreCollection<TKey1, TRecord1> GetCollection<TKey1, TRecord1>(string name, VectorStoreCollectionDefinition? definition = null)
            => throw new NotSupportedException();
