...
internal sealed partial class ReportCommand(ILogger logger, TelemetryHelper telemetryHelper)
{
            ...
            operation: () =>
                telemetryHelper.ReportOperationAsync(
                    ...
                    operation: async ValueTask () =>
                    {
                        ...
                        var overallUsageDetails = new UsageDetails();
                        var wellKnownHostUsageDetails = new UsageDetails();
                        var localMachineHostUsageDetails = new UsageDetails();
                        var usageDetailsByModel = new Dictionary<(string? model, string? modelProvider), UsageDetails>();
    ...
    private void ReportScenarioRun(
        ...
        UsageDetails overallUsageDetails,
        UsageDetails wellKnownHostUsageDetails,
        UsageDetails localMachineHostUsageDetails,
        Dictionary<(string? model, string? modelProvider), UsageDetails> usageDetailsByModel)
    {
        if (result.ChatDetails?.TurnDetails is IList<ChatTurnDetails> turns)
        {
            foreach (ChatTurnDetails turn in turns)
            {
                if (turn.Usage is not null)
                {
                    ...
                    if (!usageDetailsByModel.TryGetValue(key, out UsageDetails? usageDetails))
                    {
        ...
        Dictionary<(string? model, string? modelProvider), UsageDetails> usageDetailsByModel)
    {
        ...
        foreach (((string? model, string? modelProvider), UsageDetails usageDetails) in usageDetailsByModel)
        {
