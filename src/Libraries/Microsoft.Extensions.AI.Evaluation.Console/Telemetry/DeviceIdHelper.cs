...
// kept in sync with the linked code, so that the generated device ID remains consistent across both tools.
...
#pragma warning restore CA1031
    ...
    private static string? GetCachedDeviceId()
    {
        ...
        {
            string cacheFilePath = GetCacheFilePathForLinux();
            ReadCacheFile(cacheFilePath);
        }
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            string cacheFilePath = GetCacheFilePathForMacOS();
            ReadCacheFile(cacheFilePath);
        }

        return _deviceId;

        static void ReadCacheFile(string cacheFilePath)
        {
    ...
    private static void CacheDeviceId(string deviceId)
    {
        ...
        {
            string cacheFilePath = GetCacheFilePathForLinux();
            WriteCacheFile(cacheFilePath, deviceId);
        }
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            string cacheFilePath = GetCacheFilePathForMacOS();
        ...
        static void WriteCacheFile(string cacheFilePath, string deviceId)
        {
            string cacheFileFolderPath = Path.GetDirectoryName(cacheFilePath)!;
            _ = Directory.CreateDirectory(cacheFileFolderPath);
            File.WriteAllText(cacheFilePath, deviceId);
        }
    }

    private static string GetCacheFilePathForLinux()
    {
        ...
        string cacheFilePath = Path.Combine(cacheFileFolderPath, CacheFileName);
        return cacheFilePath;
    }

    private static string GetCacheFilePathForMacOS()
    {
