    ...
    where TDocument : notnull
{
    ...
    Task<IDatabaseResponse<TDocument>> PatchDocumentAsync(
        RequestOptions<TDocument> options,
        string id,
    ...
    /// <summary>
    /// Creates a document provided at <see cref="RequestOptions{TDocument}.Document"/>.
    /// </summary>
    /// <remarks>
    ...
    /// This includes throttling and server not available cases. </exception>
    /// <exception cref="DatabaseException">A generic exception thrown in all other not covered above cases.</exception>
    Task<IDatabaseResponse<TDocument>> CreateDocumentAsync(
        RequestOptions<TDocument> options,
        CancellationToken cancellationToken);

    /// <summary>
    /// Replaces a document having provided id with <see cref="RequestOptions{TDocument}.Document"/>.
    /// </summary>
    /// <remarks>
    /// The request will fail if a document having the provided id does not exist.
    /// If the id in the document different from the id provided in <see cref="RequestOptions{TDocument}.Document"/>,
    ...
    /// This includes throttling and server not available cases. </exception>
    /// <exception cref="DatabaseException">A generic exception thrown in all other not covered above cases.</exception>
    Task<IDatabaseResponse<TDocument>> ReplaceDocumentAsync(
        RequestOptions<TDocument> options,
        string id,
        CancellationToken cancellationToken);

    /// <summary>
    /// Tries to insert or update a document with provided document id with value provided in <see cref="RequestOptions{TDocument}.Document"/>.
    /// </summary>
    /// <remarks>
    /// This function should only be called if existence status of the target item is unknown.
    /// This is different from <see cref="UpsertDocumentAsync"/> by providing a method for resolving conflicts.
    ...
    /// This includes throttling and server not available cases. </exception>
    /// <exception cref="DatabaseException">A generic exception thrown in all other not covered above cases.</exception>
    Task<IDatabaseResponse<TDocument>> InsertOrUpdateDocumentAsync(
        RequestOptions<TDocument> options,
        string id,
    ...
    /// <summary>
    /// Upserts a document provided in <see cref="RequestOptions{TDocument}.Document"/>.
    /// </summary>
    /// <remarks>
    /// This method is suitable when existence of a document is unknown, and replace is always suitable.
    /// For conditional replace <see cref="InsertOrUpdateDocumentAsync"/> should be used instead.
    /// </remarks>
    /// <param name="options">The request options.</param>
    /// <param name="cancellationToken">The token represents request cancellation.</param>
    /// <returns>A <see cref="Task"/> containing a <see cref="IDatabaseResponse{T}"/> that wraps the updated document.</returns>
    ...
    /// This includes throttling and server not available cases. </exception>
    /// <exception cref="DatabaseException">A generic exception thrown in all other not covered above cases.</exception>
    Task<IDatabaseResponse<TDocument>> UpsertDocumentAsync(
        RequestOptions<TDocument> options,
        CancellationToken cancellationToken);
    ...
    /// This includes throttling and server not available cases. </exception>
    /// <exception cref="DatabaseException">A generic exception thrown in all other not covered above cases.</exception>
    Task<IDatabaseResponse<bool>> DeleteDocumentAsync(
        RequestOptions<TDocument> options,
        string id,
        CancellationToken cancellationToken);
    ...
    /// This includes throttling and server not available cases. </exception>
    /// <exception cref="DatabaseException">A generic exception thrown in all other not covered above cases.</exception>
    Task<IDatabaseResponse<IReadOnlyList<IDatabaseResponse<TDocument>>>> ExecuteTransactionalBatchAsync(
