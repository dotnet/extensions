        ...
        where TRecord : IngestionChunkVectorRecord, new()
    {
        ...
        VectorStoreCollectionDefinition additiveDefinition = new()
        {
            Properties =
            {
                ...
                // By using string as the type here we allow the vector store's embedding generator
                // to convert the serialized content string to the actual vector type it supports.
                new VectorStoreVectorProperty(nameof(IngestionChunkVectorRecord.Embedding), typeof(string), dimensionCount)
                {
