...
2. **publish-release** -- Stage 1 land on `internal/release/<major>.<minor>` (gated on a green official build); Stage 2 publish to nuget.org and **promote the shipping build to the public `.NET <major>` channel** (required -- this is what puts symbols on msdl). Operational and irreversible.
3. **validate-release** -- Stage 1 verify Source Link/symbols on msdl until every library package is `valid` (the release sign-off gate); Stage 2 reconcile internal -> public `release/<major>.<minor>` -> `main`; Stage 3 confirm the partner team's support-page update.
