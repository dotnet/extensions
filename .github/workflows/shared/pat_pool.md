    ...
    steps:
      - id: select-pat-number
        ...
        run: |
          ...
          # Select a random index. Seed only when a non-negative integer is provided,
          # normalized to Bash's 0-32767 $RANDOM seed range -- assigning a non-integer to
          # $RANDOM would break the arithmetic expansion below.
          if printf '%s' "$RANDOM_SEED" | grep -Eq '^[0-9]+$'; then
...
import-schema:
  ...
  random_seed:
    ...
    required: false
    description: >-
      Optional non-negative integer seed for deterministic PAT selection. Non-integer
      values are ignored and the selection falls back to Bash's default randomness.
---
