...
public class HttpStandardHedgingResilienceOptionsCustomValidatorTests
{
    ...
    public static IEnumerable<object[]> GetOptions_ValidOptions_EnsureNoErrors_Data
    {
        get
        {
            ...
            options.EndpointOptions.TimeoutOptions.TimeoutInterval = options.TotalRequestTimeoutOptions.TimeoutInterval;
            ...
            options.EndpointOptions.CircuitBreakerOptions.SamplingDuration =
                TimeSpan.FromMilliseconds(options.EndpointOptions.TimeoutOptions.TimeoutInterval.TotalMilliseconds * 2) + TimeSpan.FromMilliseconds(10);
            yield return new object[] { options };

            options = new HttpStandardHedgingResilienceOptions();
            options.HedgingOptions.MaxHedgedAttempts = 1;
            options.HedgingOptions.HedgingDelay = options.TotalRequestTimeoutOptions.TimeoutInterval;
            yield return new object[] { options };

            options = new HttpStandardHedgingResilienceOptions();
            options.HedgingOptions.HedgingDelay = TimeSpan.FromDays(1);
            options.HedgingOptions.HedgingDelayGenerator = _ => TimeSpan.FromDays(1);
            yield return new object[] { options };
        }
    }
    ...
    public static IEnumerable<object[]> GetOptions_InvalidOptions_EnsureErrors_Data
    {
        get
        {
            ...
            options.EndpointOptions.TimeoutOptions.TimeoutInterval = TimeSpan.FromSeconds(3);
            yield return new object[] { options };
