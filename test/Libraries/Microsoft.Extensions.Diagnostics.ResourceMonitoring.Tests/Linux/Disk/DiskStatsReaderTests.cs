...
public class DiskStatsReaderTests
{
    ...
    public void ReadAll_Test_Valid_DiskStats()
    {
        ...
        Assert.Equal(6_090_587ul, disk1.ReadsCompleted);
        Assert.Equal(689_465ul, disk1.ReadsMerged);
        Assert.Equal(1_120_208_521ul, disk1.SectorsRead);
        Assert.Equal(1_810_566ul, disk1.TimeReadingMs);
        Assert.Equal(19_069_165ul, disk1.WritesCompleted);
        Assert.Equal(8_947_684ul, disk1.WritesMerged);
        Assert.Equal(406_356_430ul, disk1.SectorsWritten);
        Assert.Equal(3_897_150ul, disk1.TimeWritingMs);
        Assert.Equal(0ul, disk1.IoInProgress);
        Assert.Equal(38_134_844ul, disk1.TimeIoMs);
        Assert.Equal(6_246_643ul, disk1.WeightedTimeIoMs);
        Assert.Equal(69_106ul, disk1.DiscardsCompleted);
        Assert.Equal(0ul, disk1.DiscardsMerged);
        Assert.Equal(271_818_368ul, disk1.SectorsDiscarded);
        Assert.Equal(23_139ul, disk1.TimeDiscardingMs);
        Assert.Equal(1_659_742ul, disk1.FlushRequestsCompleted);
        Assert.Equal(515_787ul, disk1.TimeFlushingMs);

        var disk2 = dictionary["dm-8"];
        Assert.Equal(100_601ul, disk2.ReadsCompleted);
        Assert.Equal(0ul, disk2.ReadsMerged);
        Assert.Equal(2_990_980ul, disk2.SectorsRead);
        Assert.Equal(23_940ul, disk2.TimeReadingMs);
        Assert.Equal(3_097_278ul, disk2.WritesCompleted);
        Assert.Equal(0ul, disk2.WritesMerged);
        Assert.Equal(32_037_680ul, disk2.SectorsWritten);
        Assert.Equal(1_410_540ul, disk2.TimeWritingMs);
        Assert.Equal(0ul, disk2.IoInProgress);
        Assert.Equal(5_488_608ul, disk2.TimeIoMs);
        Assert.Equal(1_434_496ul, disk2.WeightedTimeIoMs);

        var disk3 = dictionary["sda"];
        Assert.Equal(0ul, disk3.ReadsCompleted);
        Assert.Equal(0ul, disk3.ReadsMerged);
        Assert.Equal(0ul, disk3.SectorsRead);
        Assert.Equal(0ul, disk3.TimeReadingMs);
        Assert.Equal(0ul, disk3.WritesCompleted);
        Assert.Equal(0ul, disk3.WritesMerged);
        Assert.Equal(0ul, disk3.SectorsWritten);
        Assert.Equal(0ul, disk3.TimeWritingMs);
