...
public class ServiceEnricherExtensionsTests
{
    ...
    [Fact]
    public void ServiceMetricEnricher_GivenAnyNullArgument_Throws()
    {
        Assert.Throws<ArgumentNullException>(() =>
                ((IServiceCollection)null!).AddServiceMetricEnricher());

        Assert.Throws<ArgumentNullException>(() =>
                ((IServiceCollection)null!).AddServiceMetricEnricher(_ => { }));

        Assert.Throws<ArgumentNullException>(() =>
                ((IServiceCollection)null!).AddServiceMetricEnricher(Mock.Of<IConfigurationSection>()));

        Assert.Throws<ArgumentNullException>(() =>
            new ServiceCollection().AddServiceMetricEnricher((IConfigurationSection)null!));
    }

    [Fact]
    public void HostMetricEnricher_GivenNoArguments_RegistersInDI()
    {
        // Arrange & Act
        using var host = FakeHost.CreateBuilder()
            .ConfigureServices(services => services.AddServiceMetricEnricher())
            .Build();

        // Assert
        Assert.NotNull(host.Services.GetRequiredService<IMetricEnricher>());
        var options = host.Services.GetRequiredService<IOptions<ServiceMetricEnricherOptions>>();
        Assert.NotNull(options);
    ...
    [Fact]
    public void ServiceMetricEnricher_GivenOptions_RegistersInDI()
    {
        // Arrange & Act
        using var host = FakeHost.CreateBuilder()
            .ConfigureServices(services => services
                .AddServiceMetricEnricher(e =>
                {
                    e.ApplicationName = false;
                    e.EnvironmentName = false;
                    e.BuildVersion = false;
        ...
        // Assert
        Assert.NotNull(host.Services.GetRequiredService<IMetricEnricher>());
        var options = host.Services.GetRequiredService<IOptions<ServiceMetricEnricherOptions>>().Value;
        Assert.NotNull(options);
        Assert.False(options.ApplicationName);
        Assert.False(options.EnvironmentName);
        Assert.False(options.BuildVersion);
    ...
    [Fact]
    public void ServiceMetricEnricher_GivenConfiguration_RegistersInDI()
    {
        // Arrange & Act
        using var host = FakeHost.CreateBuilder()
            .ConfigureAppConfiguration(
                ...
                ("Serviceenrichersection:DeploymentRing", "true"))
            .ConfigureServices((context, services) => services
                .AddServiceMetricEnricher(context.Configuration.GetSection("Serviceenrichersection")))
            .Build();

        // Assert
        var enricher = host.Services.GetRequiredService<IMetricEnricher>();
        Assert.NotNull(enricher);
        Assert.IsType<ServiceMetricEnricher>(enricher);
        var options = host.Services.GetRequiredService<IOptions<ServiceMetricEnricherOptions>>().Value;
        Assert.NotNull(options);
        Assert.True(options.ApplicationName);
        Assert.False(options.EnvironmentName);
